...
clone / commit / push / **conflict-resolve** round-trip — is driven against a scratch `loreserver`,
no human involved. It reproduces the two-clone same-line conflict and asserts *Keep my version*
preserves the user's line (through the app's own `resolve_side_arg`), so a flip in the mapping or in
lore's semantics fails the build instead of silently losing work. Each test spins up an isolated
