...
**End-to-end harness (`src-tauri/tests/lore_e2e.rs`).** What the fixtures can't cover — the actual
clone / commit / push / **conflict-resolve** round-trip, and a `kill -9` **durability/consistency**
check — is driven against a scratch `loreserver`, no human involved. It reproduces the two-clone
same-line conflict and asserts *Keep my version* preserves the user's line (through the app's own
`resolve_side_arg`), so a flip in the mapping or in lore's semantics fails the build instead of
silently losing work; and it hard-crashes the server mid-write to prove the repo stays consistent
and usable (not a trapped branch) afterwards. Each test spins up an isolated
server on an ephemeral port and tears it down; the tests **skip cleanly** when the binaries aren't
present, so `cargo test` stays green everywhere. Point it at binaries with `ALT_LORE_TEST_LORE` /
`ALT_LORE_TEST_LORESERVER` (default `~/.local/bin/{lore,loreserver}`); run just it with
