 ...
 *
 * The tabs scroll (see ArrowScroller — born from ten workspaces pushing the tail off-screen);
 * the Repos label and the + Clone / + Open buttons are pinned outside the scroll region, because
 * "add a workspace" disappearing once you have many workspaces is exactly backwards.
 */
...
export function WorkspaceTabs({
  ...
  return (
    <div className="flex shrink-0 items-center gap-1 border-b border-line bg-surface-0 px-3">
      ...
      <ArrowScroller scrollToKey={activeId} className="gap-1">
        {workspaces.map((w) => {
